...
| I2 | Implement `TransmissionFXS` | high | [SDD-hardware](../team-lib/planning/sdd/SDD-hardware.md) §2 + ADR-019 pattern | I1 | TalonFXS-backed lean peer (via `ControllerTalonFXS`), scoped to its control surface; 10 acceptance tests pass; SDD-hardware updated (FXS real in §2/§3, rev→0.5); full suite green; review Blocker fixed (follow-status reported) | `done` |
