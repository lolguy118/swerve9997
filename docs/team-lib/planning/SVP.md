...
| NullAway (Error Prone plugin) | Adopted (warning) | Runs on the `@NullMarked` `api`, `vendor`, `bridge`, `hardware`, `control`, and `subsystem` layers per [ADR-018](adr/ADR-018-null-safety-annotation-policy.md); extend `AnnotatedPackages` to `auto`, promote warning→gate when a layer is clean |
