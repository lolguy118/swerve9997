...
| NullAway (Error Prone plugin) | Adopted (warning) | Runs on the `@NullMarked` `com.team271.lib.api` layer per [ADR-018](adr/ADR-018-null-safety-annotation-policy.md); extend `AnnotatedPackages` layer by layer, promote warning→gate when a layer is clean |
