...
| 6 | Reserved ADR — null-safety annotation policy | medium | Author ADR (annotation-set choice + rollout plan) | `done` | Pass 2: [ADR-018](../team-lib/planning/adr/ADR-018-null-safety-annotation-policy.md) — JSpecify + NullAway, layered rollout; unblocks the SVP NullAway gate. **Accepted (owner 2026-06-19)**; rollout → impl loop |
...
| 8 | Reserved ADR — unlimited followers in `TransmissionBase` | medium | Author ADR lifting the 4-motor cap; impl loop implements the code change to match | `done` | Pass 3: [ADR-019](../team-lib/planning/adr/ADR-019-lift-transmission-motor-cap.md) — additive `addFollower()` API, `mAllControllers` as source of truth. **Accepted (owner 2026-06-19)**; impl loop implements |
...
| DR-2 | SDD-hardware §2 Scope names `TransmissionFXS` (no transmission class on disk; `ControllerTalonFXS` does exist) | 1 | high | **RESOLVED (owner 2026-06-19): implement it.** Marked **planned** in SDD-hardware §2 (TalonFXS-backed peer of `TransmissionFX`); queued as an impl-loop item | `done` |
