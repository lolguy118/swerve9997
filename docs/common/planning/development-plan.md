...
**shall** adopt this five-phase model. Phase boundaries are events
(a competition happens, a release is cut), not calendar dates.

### 3.1 Offseason (post-championship through early preseason)

- **Entry:** competition season concluded.
- **Deliverables:** architectural ADRs, breaking API changes,
  refactors, new features.
- **Exit:** API freeze declared.

### 3.2 Preseason (API freeze through kickoff)

- **Entry:** API freeze declared.
- **Deliverables:** new features that extend (never break) the API;
  documentation updates; integration tests.
- **Exit:** build season starts; MINOR version tagged.

### 3.3 Build Season (kickoff through first competition)
...
### 3.4 Competition Season (first competition through championship)
...
- **Deliverables:** **hotfix only** — fixes to bugs discovered at
  ...
  and ≥1 maintainer approval.
- **Exit:** championship concluded. PATCH tags applied per fix.

### 3.5 Postseason (championship through offseason start)
...
### 4.1 Version Tag Events (sample)

| Event | Action | Example |
| ----- | ------ | ------- |
| Offseason start | Tag prior season final | `v<yr>.N.P` |
| Preseason API freeze | Tag next-season MINOR=0 | `v<yr+1>.0.0` |
| Each competition hotfix | Tag patch | `v<yr+1>.0.P` |

### 4.2 Phase Transition Events (sample)
