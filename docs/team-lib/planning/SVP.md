...
| NullAway (Error Prone plugin) | Adopted (warning) | Runs on the `@NullMarked` `api`, `vendor`, `bridge`, `hardware`, and `control` layers per [ADR-018](adr/ADR-018-null-safety-annotation-policy.md); extend `AnnotatedPackages` to `subsystem`/`auto`, promote warning→gate when a layer is clean |
